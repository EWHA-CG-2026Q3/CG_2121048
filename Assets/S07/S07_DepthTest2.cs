...
public class S07_DepthTest2 : MonoBehaviour
{
    ...
    // 삼각형 3
    [SerializeField] private Vector3 vertexA3 = new Vector3(210, 220, 0.7f);
    [SerializeField] private Vector3 vertexB3 = new Vector3(160, 50, 0.7f);
    [SerializeField] private Vector3 vertexC3 = new Vector3(250, 50, 0.7f);
    ...
    private void DrawTriangle(Vector3 a, Vector3 b, Vector3 c, Color color)
    {
        ...
        for (int x = 0; x < canvasWidth; x++)
        {
            for (int y = 0; y < canvasHeight; y++)
            {
                ...
                {
                    // TODO 1 완성:
                    // barycentric coordinate를 이용해 현재 픽셀의 z를 보간
                    float interpolatedZ =
                        w1 * a.z +
                        w2 * b.z +
                        w3 * c.z;
                    ...
                    // TODO 2 완성:
                    // 기존에 기록된 z보다 가까울 때만 색과 depth 갱신
